	...
	<div class="page-container">
		...
		<div id="content">
			<img src="${basePath}/images/tiaofu_03.jpg" style="margin-bottom:5px;" width="1000">
			  ...
			  <div id="left1">
			  <h2 class="h2">
			  	&nbsp;&nbsp;<a href="/">首页</a> > <a href="${basePath}/j002_init.ac?tid=${pid}&pid=${pid}">${typeBeanP.name}</a><c:if test="${typeBeanP.name!=typeBean.name}"> > ${typeBean.name}</c:if>
			  </h2>
			  <div class="container">
				 <c:choose>
				  	<c:when test="${bean.detail_info!=null}">${bean.detail_info}</c:when>
				  	<c:otherwise>信息不存在或已删除</c:otherwise>
				  </c:choose>
			  </div>
